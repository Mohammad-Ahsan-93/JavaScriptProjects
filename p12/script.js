...
// On page load, focus on the text field so user can type
text.focus();
...
// All Functions
// 1. Generate a random word from wordlist
function generateRandonWord() {
    return wordList[ Math.floor(Math.random() * wordList.length) ];
}

// 2. Function to Add the Random Word to the DOM
function addWordToDOM() {
    randomWord = generateRandonWord();
...
function updateTime() {
    ...
    if ( time === 0 ) {
        ...
        // End the game by showing end game container
        gameOver();
    }
}

// 5. Function for Game Over
function gameOver() {
...
text.addEventListener('input', e => {
    ...
    // Check if user inout matches random word
